...
/**
 * TWAMP Stop-N-Sessions (variable length). RFC 5938 §3.4
 *
...
class StopNSession {
    ...
    fun writeTo(bb: ByteBuffer, mode: TwampMode) {
        ...
        bb.put(TwampCommand.STOP_N_SESSION)
    ...
    companion object {
        fun readFrom(bb: ByteBuffer, mode: TwampMode): StopNSession {
            ...
            if (mode.isControlEncrypted()) {
                ...
                if (!hmac.contentEquals(expected)) {
                    throw SecurityException("HMAC validation failed for Stop-N-Sessions")
                }
            }
            return StopNSession().also { it.sids = sids }
        }
    }
}
