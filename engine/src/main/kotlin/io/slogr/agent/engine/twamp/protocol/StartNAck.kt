...
/**
 * TWAMP Start-N-Ack (variable length). RFC 5938 §3.3
 *
 * Wire layout (sent by reflector → controller, no leading command byte for dispatcher,
 * but we write the type byte when serializing):
 *    1 byte  type = 8
 *    1 byte  accept
 *    2 bytes MBZ1
 *    8 bytes MBZ2
...
class StartNAck {
    ...
    fun writeTo(bb: ByteBuffer, mode: TwampMode) {
        ...
        bb.put(TwampCommand.START_N_ACK)
        bb.put(accept)
        bb.putShort(0)  // MBZ1
        bb.putLong(0L)  // MBZ2
        bb.putInt(sids.size)
    ...
    companion object {
        fun readFrom(bb: ByteBuffer, mode: TwampMode): StartNAck {
            ...
            if (mode.isControlEncrypted()) {
                ...
                if (!hmac.contentEquals(expected)) {
                    throw SecurityException("HMAC validation failed for Start-N-Ack")
                }
            }
            return msg
        }
