...
import io.slogr.agent.native.JniProbeAdapter
...
fun run(args: Array<String>): Int {
    ...
    // ── Transport: prefer JNI (raw sockets, kernel timestamps, fast traceroute)
    //    Fall back to Java for Windows/macOS or when native lib is missing.
    val adapter = try {
        JniProbeAdapter().also { System.err.println("[slogr] Transport: JniProbeAdapter (native)") }
    } catch (_: UnsatisfiedLinkError) {
        JavaUdpTransport().also { System.err.println("[slogr] Transport: JavaUdpTransport (fallback)") }
    }
